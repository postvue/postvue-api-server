...
public class PostLikeNotificationProcessService
	...
	public void processNotification(SnsPost snsPost, SnsPostUserReaction snsPostUserReaction) {
		Optional<PostLikeNumDao> postLikeNumDaoOpt = snsPostUserReactionRepository.findLikeNumWithoutMe(
			snsPost.getId(), snsPost.getSnsUser().getId());

		Integer postLikeNum = postLikeNumDaoOpt.isPresent() ? postLikeNumDaoOpt.get().getLikeCount() :
			SnsNotificationConst.ZERO_NOTIFICATION_NUM;

		boolean needNotification = NotificationUtils.isNotificationInSequence(postLikeNum,
			SnsNotificationConst.POST_LiKE_NOTIFICATION_MIN_NUM,
			SnsNotificationConst.POST_LiKE_NOTIFICATION_SEQUENCE_NUM);
		if (!needNotification) {
			return;
		}

		// 이미 같은 알림이 있는 경우 알림하지 않음
		if (getHasAlreadyNotification(snsPost, postLikeNum)) {
			return;
		}

		SnsNotification snsNotification = saveNotification(snsPost, snsPostUserReaction, postLikeNum);
	...
	public Boolean getHasAlreadyNotification(SnsPost snsPost, Integer notificationCount) {
		return notificationServiceTemplate.getHasAlreadyNotificationByPost(snsPost, notificationCount,
			SnsNotificationType.POST_LIKE_NOTIFICATION);
	}
	...
	public SnsNotification saveNotification(SnsPost snsPost, SnsPostUserReaction snsPostUserReaction,
		...
		SnsNotification snsNotification = notificationServiceTemplate.convertPostNotification(snsPost,
			snsUser,
			SnsNotificationType.POST_LIKE_NOTIFICATION, Collections.singletonList(
