...
public class SnsTagEndpoint implements CrudService<SnsTagEndpointDto, Long> {
	private final SnsTagEndpointService snsTagEndpointService;

	@Override
	@Nonnull
	public List<@Nonnull SnsTagEndpointDto> list(Pageable pageable, @Nullable Filter filter) {
		try {
			return snsTagEndpointService.listProcess(pageable,filter).stream().map((SnsTagEndpointDto::fromEntity)).toList();
		}
		catch (Exception e){
	...
	public @Nullable SnsTagEndpointDto save(SnsTagEndpointDto value) {
		try {
			return SnsTagEndpointDto.fromEntity(snsTagEndpointService.saveProcess(value));
		}
		catch (Exception e){
			log.error(e.getMessage());throw new EndpointException("서버 오류 발생", LogTemplateConst.getLogInfoTemplate(e.getMessage() + "_" + e.toString(), LocalDateTime.now().toString()));
		}
	}
