...
public interface SnsPostCommentReactionRepository extends JpaRepository<SnsPostCommentReaction, Long>,
	...
	@Query("SELECT "
		...
		+ "(SELECT COUNT(*) FROM SnsPostCommentReaction CR WHERE CR.sourceComment.id = SPCR.id AND CR.isCommented = TRUE) AS commentCount, "
		...
		+ "WHERE SPCR.snsPost.id = :snsPostId AND SPCR.isSource = TRUE AND SPCR.isCommented = TRUE "
		+ "AND SPCR.id < :snsPostCommentReactionId "
		+ "GROUP BY SPCR.id, "
		+ "SPCR.commentMediaType, SPCR.commentMediaContent, SPCR.commentMsg, "
		+ "IS_LIKE_TB.isLiked, "
		+ "SPCR.createdAt, COMMENT_USER.profilePath, COMMENT_USER.id "
	...
	@Query("SELECT "
		...
		+ " "
		+ "LEFT OUTER JOIN SnsPostCommentLike AS COMMNET_LIKE "
		+ "ON SPCR.id = COMMNET_LIKE.snsPostCommentReaction.id "
		+ " "
		+ "LEFT OUTER JOIN SnsPostCommentReaction AS SOURCE_COMMENT "
	...
	@Query(value = "SELECT SNS_PCR FROM SnsPostCommentReaction SNS_PCR WHERE "
		+ "SNS_PCR.id = :snsPostCommentReactionId AND SNS_PCR.commentUser.id = :snsUserId")
