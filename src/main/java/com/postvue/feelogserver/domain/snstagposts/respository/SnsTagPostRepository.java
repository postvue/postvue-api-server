...
public interface SnsTagPostRepository extends JpaRepository<SnsTagPost, Long>, JpaSpecificationExecutor<SnsTagPost> {
	...
	String RECOMM_TAG_NATIVE_QUERY = "with "
		...
		+ "SELECT SNS_T.sns_tag_id AS TAG_ID, SNS_T.tag_name AS TAG_NAME, SNS_T.tag_reps_batch_content AS tag_reps_batch_content, SNS_T.tag_reps_batch_content_type FROM tag_id_by_recomm INNER JOIN sns_tags_tb AS SNS_T ON tag_id_by_recomm.sns_tag_id = SNS_T.sns_tag_id LIMIT :pageNumByTagRecomm";
	...
	List<SnsRecommTagDao> findRecommTagList(
		...
		@Param("pageNumByTagRecomm") Integer pageNumByTagRecomm);
