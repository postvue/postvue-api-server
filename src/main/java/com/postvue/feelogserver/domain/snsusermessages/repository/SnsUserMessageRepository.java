...
public interface SnsUserMessageRepository extends JpaRepository<SnsUserMessage, Long>, JpaSpecificationExecutor<SnsUserMessage> {
	...
	String MSG_INBOX_MESSAGES_QUERY = "WITH "
		...
		+ "MSG_IT.unread_count, "
		+ "LATEST_MESSAGES.sns_user_message_id AS latest_msg_id, "
		+ "LATEST_MESSAGES.msg_media_type AS latest_msg_media_type, "
		+ "LATEST_MESSAGES.msg_media_content AS latest_msg_media_content, "
		+ "LATEST_MESSAGES.msg_text_content AS latest_msg_text_content, "
		+ "LATEST_MESSAGES.msg_meta_info AS latest_msg_meta_info, "
		+ "LATEST_MESSAGES.created_at AS posted_at "
		+ "FROM LATEST_MESSAGES "
		...
		+ "FROM ROOM_LIST_WITH_RECENT_MSG_TB AS ROOM_LWRM "
		+ "INNER JOIN MY_ROOM_LIST_TB ON ROOM_LWRM.msg_room_id = MY_ROOM_LIST_TB.msg_room_id "
		+ "LEFT JOIN sns_block_users_tb AS SNS_BU "
		+ "ON (SNS_BU.sns_blocker_user_id = :snsUserId AND SNS_BU.sns_blocked_user_id = MY_ROOM_LIST_TB.target_user_id) "
		+ "OR (SNS_BU.sns_blocker_user_id = MY_ROOM_LIST_TB.target_user_id AND SNS_BU.sns_blocked_user_id = :snsUserId) "
	...
	@Query(
		"SELECT new com.postvue.feelogserver.domain.snsusermessages.dao.MsgConversationDao("
			...
			+ "snsUserMessage.msgTextContent, "
			+ "snsUserMessage.msgMediaType, "
			+ "snsUserMessage.msgMediaContent, "
			+ "snsUserMessage.msgMetaInfo, "
